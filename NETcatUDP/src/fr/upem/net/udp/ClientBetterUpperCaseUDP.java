...
public class ClientBetterUpperCaseUDP {
	...
	public static Optional<ByteBuffer> encodeMessage(String msg, String charsetName) {
		var buffer = ByteBuffer.allocate(MAX_PACKET_SIZE);// naturally in BIGENDIAN
		var cs = Charset.forName(charsetName);
		var asciics = ASCII_CHARSET.encode(charsetName);
		if (asciics.remaining() + Integer.BYTES + msg.getBytes().length > 1024) {
			return Optional.empty();
		}
		buffer.putInt(asciics.remaining());
		buffer.put(asciics);
		buffer.put(cs.encode(msg));
	...
	public static Optional<String> decodeMessage(ByteBuffer buffer) {
		...
		buffer.limit(size + Integer.BYTES);

		try {
			var cs = Charset.forName(ASCII_CHARSET.decode(buffer).toString());
			buffer.limit(limit);
